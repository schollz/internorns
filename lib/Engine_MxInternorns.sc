	...
	*new { arg context, doneCallback;
	...
	// break live
	   ...
	   context.server.sync;
		...
		this.addCommand("amp","iff", { arg msg;
		...
		SynthDef("bass", {
			arg out,amp=0,attack=0.01,decay=2,
			note=35,t_trig=1,lpf=2;
			var snd,env;
			env=EnvGen.ar(Env.perc(attack,decay),gate:t_trig,doneAction:2);
			amp=Lag.kr(amp,2);
			lpf=Lag.kr(lpf,2);
		...
		this.addCommand("bassnote","ffff", {arg msg;
			if (synBass==nil,{
				synBass=Synth("bass", [\amp,0,\out,0,\note,msg[1],\amp,msg[2],\attack,msg[3],\decay,msg[4]],target:context.xg);
				NodeWatcher.register(synBass);
			},{
				if (synBass.isRunning,{
					synBass.set(\t_trig,1,\note,msg[1],\amp,msg[2],\attack,msg[3],\decay,msg[4]);
				},{
					synBass=Synth("bass", [\amp,0,\out,0,\note,msg[1],\amp,msg[2],\attack,msg[3],\decay,msg[4]],target:context.xg);
					NodeWatcher.register(synBass);
				});
